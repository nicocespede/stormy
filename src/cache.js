...
const { DEV_ENV, LOCAL_ENV, CONSOLE_GREEN, CONSOLE_RED, GERMANY_HOURS_OFFSET } = require('./constants');
const { convertTime, consoleLog, logToFile, logToFileError, consoleLogError, convertToUTCFromLocal, convertToUTCFromGermany } = require('./util');
...
const updateKruMatches = async type => {
    ...
    try {
        ...
        a.each((_, el) => {
            ...
            const date = convertToUTCFromGermany(new Date(`${split.shift().replace(/\//g, '-')}T${convertTime(split.pop())}Z`));
